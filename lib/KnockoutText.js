    ...
    onLoaded() {
        ...
        setTimeout( () => {
            ...
            if ( 'ontouchstart' in window ) {
                ...
                let lastViewportSize = this.winSize
                let height = this.container.clientHeight
                let cancelScroll = ( e ) => e.preventDefault()
                this.swipeUp = false
                window.addEventListener( 'resize', ( e ) => {
                    if ( this.winSize[ 1 ] < window.innerHeight ) {
                        if ( !this.container.classList.contains( 'about-pane' ) ) {
                            this.el.innerHTML = 'Nice.'
                            setTimeout(go, 1000)
                        }
                        document.body.classList.add( 'full' )
                        document.body.addEventListener( 'touchmove', cancelScroll )
                        this.emit( 'resize' )
                    } else {
                        document.body.classList.remove( 'full' )
                        document.body.removeEventListener( 'touchmove', cancelScroll )
                    }
                } )
            } else {
                go()
